...
class OpenAIProvider(BaseProvider):
        ...
        try:
            ...
            if use_responses:
                try:
                    ...
                    if ("eof" in error_str or "connection" in error_str or "unexpected" in error_str
                            or "404" in str(e) or "not found" in str(e).lower()
                            or "400" in str(e) or "account invalid" in str(e).lower()
                            or "invalid_argument" in str(e).upper()):
                        logger.warning(
                            f"[Responses Generate] Gateway error ({str(e)[:100]}), "
        ...
        try:
            ...
            if use_responses:
                try:
                    ...
                    error_str = str(e)
                    if ("__discriminator__" in error_str or "__dict__" in error_str
                            or "404" in error_str or "not found" in error_str.lower()
                            or "400" in error_str or "account invalid" in error_str.lower()
                            or "invalid_argument" in error_str.upper()
                            or "eof" in error_str.lower() or "connection" in error_str.lower()
                            or "unexpected" in error_str.lower()):
                        self.__class__._fallback_to_chat_cache.add(base_url)
            ...
            if hasattr(stream, "choices") and not hasattr(stream, "__aiter__"):
                content = self._extract_text_from_response(stream)
                if content:
                    yield content
                return
    ...
    async def _stream_via_chat(self, prompt: Prompt, config: GenerationConfig) -> AsyncIterator[str]:
        """Chat Completions 流式生成"""
        messages = self._build_messages(prompt)
        request_kwargs = self._build_chat_request_kwargs(messages, config, stream=True)
        stream = await self.async_client.chat.completions.create(**request_kwargs)
    ...
    async def _stream_via_responses(self, prompt: Prompt, config: GenerationConfig) -> AsyncIterator[Any]:
        """Responses API 流式生成"""
        request_kwargs = self._build_responses_request_kwargs(prompt, config, stream=True)
        stream = await self.async_client.responses.create(**request_kwargs)

        if hasattr(stream, "output") and not hasattr(stream, "__aiter__"):
            content = self._extract_text_from_responses_response(stream)
            if content:
                yield type(
                    "_FakeChunk", (),
                    {
                        ...
                        "item": type(
                            "_FakeItem", (),
                            {
                                "type": "message",
                                "content": [
                                    type("_FakePart", (), {"type": "text", "text": content})()
                                ],
                            },
                        )(),
                    },
