...
|------|------|
| 1 | 启动双端服务（后端+前端） |
| 2 | 启动后端服务（后端 API：http://localhost:8005） |
| 3 | 启动前端开发服务器（前端：http://localhost:3000） |
| 4 | Docker启动 Qdrant 向量数据库（可选，用于语义检索） |
| 5 | 运行测试 |
| 6 | 懒人安装（一键配置 Python 环境、安装依赖、下载模型） |
| 0 | 退出 |
