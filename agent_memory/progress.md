...
- 选题立项池验证：
  ...
  - 2026-04-30 新功能详细验收：
    ...
    - NovelPro 监控误报优化已完成并部署：单个“本章已登记时间事件”的时间线可疑冲突降为 `warning`，前端显示为“时间需确认”；同一测试小说线上复测为 `health=warning/76`，`error_count=0`。
    - 本地 `uv.lock` 为 `uv run` 生成的未跟踪噪音，已加入 `.gitignore`，未删除用户文件。
