...
class SharedStateRepository:
    ...
    def __init__(self, shared_dict: Optional[SharedStateDict] = None):
        """初始化共享状态仓库
    ...
    def set_shared_dict(self, shared_dict: SharedStateDict):
        """设置共享字典（用于子进程注入）"""
        self._state = shared_dict
...
def init_shared_state_repository(shared_dict: SharedStateDict) -> SharedStateRepository:
    """初始化共享状态仓库（主进程调用）"""
...
def inject_shared_dict(shared_dict: SharedStateDict):
    """注入共享字典（子进程调用）"""
    global _shared_state_repository
    if _shared_state_repository is None:
        _shared_state_repository = SharedStateRepository()
